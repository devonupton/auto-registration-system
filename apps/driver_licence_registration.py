...
class App3( Toplevel ):
    def __init__( self, userCx ):
        ...
        #Buttons
        condition_id_help_button = Button( self, text="?", \
                    command=lambda: self.findConditionIDs(), padx=0, pady=0 )
        ...
        new_person_button.grid( row=3, column=2, rowspan=2, \
    ...
    def openimage( self ):
        if PIL_loaded:
            try:
                ...
                msg = "Image could not be opened\nErr 0xa3-2"
                tm.showerror( "Open Image Error", msg )
        else:
            msg = "Module unavailable to open images\nErr 0xa3-3"
            tm.showerror( "Open Image Error", msg )
    ...
    def findConditionIDs( self ):
        askMsg = "Do you wish to bring up a table of the possible " + \
                 "Driving Conditions and their Condition IDs?"
    ...
    def submit_form( self ):
        ...
        statement = "INSERT INTO drive_licence \
                     VALUES( :licence_no, :sin, :class, \
                     :photo, :issuing_date, :expiring_date )"
        try:
            cursor.execute( statement, self.entries )
        except cx_Oracle.DatabaseError as exc:
            cursor.execute( "ROLLBACK to App3Save" ) 
            error, = exc.args
            if error.code == 1: #Licence_no exists or person already has one

                #Test if licence already exists
                statement = "SELECT licence_no FROM drive_licence \
                             WHERE licence_no = :a"
                try:
                    entry = self.entries["licence_no"].ljust(15)
                    cursor.execute( statement, a=entry )
                except: #Unknown error
                    tm.showerror( error_type, "Unexpected Error\nErr 0xa3-14" )
                    cursor.close()
                    return

                if len( cursor.fetchall() ) == 0: #Sin must be taken
        ...
        for condition_id in self.condition_id_list:
            try:
                cursor.execute( statement, \
                    a=self.entries["licence_no"], b=condition_id )
            except cx_Oracle.DatabaseError as exc:
                cursor.execute( "ROLLBACK to App3Save" ) 
                cursor.close()
                error, = exc.args
        ...
        #Success message
        successInfo = "Licence # '" + self.entries["licence_no"] + \
                      "' has been added to the database\nSIN: " + \
                      self.entries["sin"] + "\nCondition IDs: " + \
                      ", ".join( str(i) for i in self.condition_id_list )
    ...
    def validate_input( self ):
        ...
        if self.entries["licence_no"] == '' \
            or len( self.entries["licence_no"] ) > 15:
            msg = "Invalid Licence #: Must not be blank " + \
                  "and no longer than 15 characters\nErr 0xa3-5" 
            tm.showerror( error_type, msg )
            return

        #class validation
        if self.entries["class"] == '' or len( self.entries["class"] ) > 10:
            msg = "Invalid Class: Must not be blank and no " + \
        ...
        if date1 > datetime.now():
            msg = "Invalid Issuing Date: Issuing Date cannot be in the " + \
                  "future\nErr 0xa3-8"
            tm.showerror( error_type, msg )
            return
        ...
        if self.entries["sin"] == '' or len( self.entries["sin"] ) > 15:
            msg = "Invalid SIN: Must not be blank and no longer than 15 " + \
                  "characters\nErr 0xa3-11" 
            tm.showerror( error_type, msg )
            return
...
class NewCondition( Toplevel ):
    def __init__( self, parent, return_entry=None ):
        ...
        #Used to return the c_id back to the previous window
    ...
    def submit_form( self ):
        ...
        try:
            ...
            if error.code == 1: #C_id already exists
                tm.showerror( "Submit Failure", "Condition ID '" + \
                          str(self.c_id) + "' is already taken\nErr 0xa3-24" )
            else:
        ...
        #Success message
        successInfo = "Condition ID '" + str(self.c_id) + \
                      "' has been added to the database"
        tm.showinfo( "Success!", successInfo )  
    ...
    def validate_input( self ):
        ...
        if self.descr == '' or len( self.descr ) > 1024:
            msg = "Invalid Description: Must not be blank or longer than " + \
                  "1024 characters\nErr 0xa3-22"
            tm.showerror( error_type, msg )
            return
        ...
        try:
            ...
            msg = "Invalid Condition ID: Must be an integer between " + \
                  "-(2^31)-1 and (2^31)-1\nErr 0xa3-23"
            tm.showerror( error_type, msg )
            return
